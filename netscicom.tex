...
\title{Exploring Topology Dynamics of Bitorrent Network: A Temporal View}
...
Our experimental results show that the node degree of Bittorrent networks follows the power-law model only for relatively short periods, as verified with the Kolmogorov-Smirnov (KS) goodness of fit test. 
...
This popularity reflects the robustness and efficiency of the Bittorrent protocol. These characteristics of Bittorrent comefrom its peer and piece selection strategies to distribute large files efficiently. 
...
The Bittorrent protocol does not offer peer discovery and the tracker also does not know about topologies since peers never send information to the tracker concerning their connectivity with other peers. 
...
This surprising result may contradict earlier findings, although our simulations demonstrate this same phenomenon, suggesting that these results are not simply measurement artifacts.

%------------BACKGROUND and RELATED WORK -------------------
\section{Bittorrent Background}\label{background}
...
PEX is a mechanism introduced in Bittorrent to reduce the load on the trackers.  
...
Even though most Bittorrent clients support PEX,  there is no PEX specification, only a kind of informal understanding among Bittorrent client developers.
PEX messages are exchanged between peers every 30 seconds.  
However, some differences exist; for example, for some Bittorrent clients derived from rasterbar libtorrent \cite{rasterbar}, the PEX message can only contain a maximum of a hundred IP address and port pairs. 
...
We are using peer exchange (PEX) to collect peer neighbors information and describe the network formed in terms of properties such as node degree and average clustering. 
...
In these simulations, we assumed that peer arrivals and departure (churn) follow an exponential distribution as explained by Guo \textit{et al}. \cite{guo2005measurements}. 
...
We crawled top 30 TV series torrent from thepiratebay which claimed as the biggest torrent tracker on the Internet.
...
PEX messages sent from old versions of Azureus Bittorrent client contain not only currently connected peers but also its historically connected peers. 
We can distinguished Azureus Bittorrent client from log files.
To cope with this situation, we do not include old Azureus client in data processing. 
...
\caption{CDF number of peers for all data sets. It is clearly show high variation number of peers in every swarm.} 
\label{fig:num_peers}
\end{figure}
...
The scaling parameter usually lies
in the range $1.8<\alpha<3.5$.
...
The $p$ value more than $0.1$ maximum is only around $60\%$ therefore It clearly shows that node degree in Bittorrent topology does not always follow power-law distribution.
...
Figure \ref{fig:cdf-lr} shows the cummulative distribution function of  $\rho$ value. 
...
This sparse observation clearly describe that comparing the models to other models is very complex task in highly dynamics networks.
...
The disctinction between a random and a non-random graph can be measured by clustering-coefficient \cite{watts1998collective}.
Newman \cite{newman2003properties} mentions that virus outbreaks spread faster in highly clustered networks. A network that has a high clustering coefficient and a small average path length is called a \textit{small-world} model \cite{watts1998collective}.
...
It is between $70\% - 90\%$ the clustering coefficient values under $0.1$, while one data set show only $40\%$. 
...
The huge numbers of peers send P2P download request to a random target on the Internet and anti-P2P companies inject bogus peers through PEX was discussed in \cite{li2010measurement}. Higher upload-to-download ratios in  Bittorrent darknet was discussed in \cite{zhang2010bittorrent}.
...
Dale \textit{et al}. \cite{dale2008evolution} also showed that the path length formed in Bittorrent swarms averages four hops.  
...
First, power-law claims must be handled carefully. Many steps are required to confirm the power-law behavior, including alternative model checking, and we must be prepared for disappoinment since other models may give a better fit. 
Second, our methodology relies on real work measurement combine with simulation for validation. 
We are using real swarms from a real and operational Bittorrent tracker. This real world measurement will reflect different type of clients connected to our swarm and each client has a different behavior. 
...
We conclude that further work throughoutthe community is necessary to continue to improve the agreement of simulation and controlled experiment with the real world, and that such work will impact our understanding of Bittorrent performance and its effects on the Internet.


\section*{Acknowledgements}
This research was supported by The Ministry of Education, Culture, Sports, Science
