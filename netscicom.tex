...
Our measurements, verified using the Kolmogorov-Smirnov goodness of fit test and the likelihood ratio test and confirmed via simulation, show that a power-law with exponential cutoff is more plausible model than a pure power law distribution.  
...
Our findings also give another contribution. 
We show that a power-law with exponential cut-off distribution is a better model than a pure power-law distribution. 
...
We also study the source code of two popular Bittorrent clients, uTorrent (using rasterbar libtorrent source code) and Vuze, in term of peer connectivity.
...
\caption{CDF number of peers for every swarm during measurement with x to y time samples for each torrent. It is clearly show high variation number of peers in every swarm due to churn in Bittorrent network.} 
\label{fig:num_peers}
\vspace{-2mm}
\end{figure}
...
In \emph{torrent1} the optimum $x_{min}=2$ and $\alpha=2.11$ if we put $x_{min}=1$ we will get $\alpha=2.9$ and the fit visually seems not good, while in the same graph we also show the optimum $x_{min}=1$ for \emph{torrent3} and the fit visually seems good.

Figure  \ref{fig:cdf-p} shows the CDF for $p$ values for all data sets. 
This figure shows that from the K-S statistics point of view around $45\%$ of Bitorrent networks do not follow a power-law model at any given point in time.
...
\caption{CDF plot $p$ value of K-S statistics. It shows that across our entire set of Bittorrent snapshots, around $45\%$ of the time a power-law distribution is not a good fit for the data. 
...
\caption{Scatter plot of $p$ value vs $\rho$ value. 
	Points in area 1 are topology time samples for which a power-law with exponential cut-off is a more plausible model than a power-law.
	In area 3, a pure power-law is more plausible than power-law with exponential cut-off, while in area 2 the results are ambiguous.} 
...
We divide the figure into three areas: area 1, area 2, and area 4 with green line as a border. 
...
As mentioned before $\rho$ value $<0.1$ is an indication of alternative model may be plausible for data sets.
...
In this figure, we see that no points lie in area 1 and area 2 (LR=possitive sign) instead in detail  $58\%$ data lie in area 3 and $42\%$ data lie in area 4.
This figure gives us an explanation that the alternative model is better.  
Althought in the case $p$ value $<0.1$ we reject power-law as the plausible model, the alternative model is still better than the power-law model. 
...
\caption{CDF of the clustering coefficient for each torrent. It is clearly show that Bittorrent networks have low clustering coefficient.} 
