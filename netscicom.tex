...
Our results show that the node degree of Bittorrent networks follows power-law with exponential cut-off, as verified with the Kolmogorov-Smirnov (KS) goodness of fit test and likelihood ratio test.
We also found that the average clustering coefficient is very low. 
This is indication that Bitorrent network may form  power-law with exponential cut-off random network.
...
Our findings in this work shed light into puzzling that  Bittorrent networks can be described by power-law with exponential cut-off random networks.
...
This figure explain that from KS statistics point of view around $45\%$ Bitorrent networks do not follow power-law model.
...
Even if we have estimated the power-law parameter properly and the fit is decent, it does not mean power-law model is good.
There are always possibilities that non power-law models better than power-law model.
We use the likelihood ratio test \cite{vuong1989likelihood} to see whether other distributions can give better parameter estimation and the fit is decent.
...
Vuong \cite{vuong1989likelihood} provides foundation os the significance value ($\rho$ value) for likelihood ratio test. 
...
Figure \ref{fig:scatter-pvalue-vs-rho} shows scatter plot our data: $p$ value vs $\rho$ value.
$52\%$ points lie in range $\rho < 0.1$ and $p > 0$.
We do not count LR value in that figure instead at least $\rho < 0.1$ is indication that alternative model gives better fit rather than pure power-law model.
Now we plot $p$ value vs LR as shown in figure \ref{fig:scatter-pvalue-vs-lr-for-rho-le-01} for $\rho < 0.1$. 
In this figure, we count that $58\%$ data lie in $LR=$negative and $p$ value $< 0.1$ and $42\%$ data lie in $LR=$negative and $p$ value $>0.1$.
This figure gives us explanation that alternative model is better. 
Althought in case $p$ value $<0.1$ which we reject power-law as model , the alternative model is still better than power-law model. 
...
This observation clearly describe that comparing the model to other models is very complex task in highly dynamics networks.
...
This low clustering coefficient observation is same as observed by Dale \textit{et al}. \cite{dale2008evolution}.
...
Unlike \cite{dale2008evolution}, our findings of this work is that the node degree of the graph formed in Bittorrent swarm can be described by power law with exponential cut-off and low clustering observation implies Bittorrent networks is near to random networks.
